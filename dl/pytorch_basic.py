...
class BasicDeepLearning(object):
    ...
    def load(self, path=None):
        if not path:
            path = "./model_weights.pth"
            pass
        self.model = NeuralNetwork().load_state_dict(torch.load(path))
        self.model.eval()
        pass
    pass
...
def app():
    ...
    # dataset: 原始数据样本加载; __init__:原始数据加载; __len__:原始数据样本数量; __getitem__:原始数据样本索引;
    # dataloader: 将原始数据样本，组织成训练所需的 可迭代 mini-batch样本集合，具有自动shuffle功能；
    pass
